...
describe("Aggregate", () => {
    ...
    it("should have id set if created as empty.", () => {
        const aggregate = Aggregate.createEmpty(aggregateMapping);
    ...
    it("should guard against inconsistency.", () => {
        expect(() => {
            ...
            Aggregate.createEmpty(aggregateMapping, mockFieldValues);
        }).toThrowError();
    });

    it("should compute changes.", () => {
        const aggregate = Aggregate.createEmpty(aggregateMapping);
        const differentAggregate = Aggregate.createEmpty(aggregateMapping);
