...
import { FIELD_AGE } from "../../../Utils/ExampleFields";

describe("StringFieldValue", () => {
    it("should have default value.", () => {
        const fieldValue = new NumberFieldValue(FIELD_AGE);
        expect(fieldValue.$value).toBe(0);
    });

    it("should have setters/getters/clone/isequal", () => {
        const fieldValue = new NumberFieldValue(FIELD_AGE, 12345);
        expect(fieldValue.$name).toBe("age");
        expect(fieldValue.$field.isEqual(FIELD_AGE)).toBe(true);
    ...
    it("should be equal if same field and value.", () => {
        const field = new NumberFieldValue(FIELD_AGE, 12345);
        const secondField = new NumberFieldValue(FIELD_AGE, 12345);
    ...
    it("should not be equal if same field but different value.", () => {
        const field = new NumberFieldValue(FIELD_AGE, 12345);
        const secondField = new NumberFieldValue(FIELD_AGE, 123456);
        expect(field.isEqual(secondField)).toBe(false);
    });
});